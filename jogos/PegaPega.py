...
while jogar:  # enquanto jogar for verdadeiro, vamos jogar
	...
	while gameloop:
		...
		# leitura do teclado
		if is_key_pressed():
			teclaPressionada = get_key_pressed()
		...
		# cálculo da Inteligência do Movimento do Inimigo
		...
		# verifica colisão
		if ((inimigoCoordenadaX==suaCoordenadaX) and (inimigoCoordenadaY==suaCoordenadaY)):
			pegou=True
	...
	if pegou:      # se houve a colisão, então mostrar o efeito da colisão
		...
		teclaPressionada=None
		while not teclaPressionada in ['s','n']: # repete até o jogador digitar "s" ou "n"
			teclaPressionada=read_key()

		if teclaPressionada == 'n': # sai do jogo se digitar "n"
			gameloop=False
